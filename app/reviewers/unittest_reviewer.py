...
from app.reviewers.reviewer_context import UnitTestReviewerContext
...
logger = logging.getLogger("codemind.reviewer.unittest")


class UnitTestReviewer(BaseReviewer):
    """
    深度层 Reviewer：基于 AST 结构和 Diff 生成单测建议。

    输入：UnitTestReviewerContext（AST 签名 + Diff）
    输出：Markdown 格式的单测建议
    """

    name = "unittest"
    fallback_message = (
        "⚠️ 详细测试分析因超时跳过，建议关注核心变动的边界条件与异常抛出。"
    )
    ...
    async def execute(self, context: UnitTestReviewerContext) -> ReviewResult:
        start_time = time.time()
        pr = context.pr

        logger.info(
            f"UnitTest Reviewer starting for {pr.owner}/{pr.repo}#{pr.pr_number}"
        )
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(
                    f"UnitTest Reviewer completed in {round(time.time() - start_time, 2)}s "
                    f"(attempt {attempt + 1})"
                )
                return self._make_result(
                ...
                logger.warning(
                    "UnitTest Reviewer task cancelled by external timeout controller."
                )
                raise  # 将 CancelledError 持续抛出，交给外部降级处理
            except Exception as e:
                logger.warning(f"UnitTest Reviewer attempt {attempt + 1} failed: {e}")
                if attempt == max_retries - 1:
                    logger.error(
                        f"UnitTest Reviewer failed after {max_retries} attempts."
                    )
                    return self._make_result(
                        ReviewerStatus.FAILED, self.fallback_message, start_time, str(e)
                    )
