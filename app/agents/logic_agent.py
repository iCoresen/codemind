...
包装原有的 Security + Performance + Reducer 多 Agent 流程。
接收 Diff 与 PR 元数据，内部并发执行安全与性能分析，
然后通过 Reducer 综合生成最终的逻辑审查报告。

保留了原有 pr_reviewer.py 中的完整多 Agent 架构设计。
"""
import time
import asyncio
import logging
...
logger = logging.getLogger("codemind.agent.logic")


class LogicAgent(BaseAgent):
    """
    核心层 Agent：包装原有的 Security + Performance + Reducer 流程。
    
    内部仍然并发执行 Security Agent 和 Performance Agent，
    然后通过 Reducer Agent 综合分析，生成最终的逻辑审查报告。
    
    输入：LogicAgentContext（Diff + PR 元数据）
    ...
    async def execute(self, context: LogicAgentContext) -> AgentResult:
        ...
        logger.info(f"Logic Agent starting for {pr.owner}/{pr.repo}#{pr.pr_number}")
        
        # ── Phase 1: 并发执行 Security + Performance 子 Agent ──
        agent_names = ["security", "performance"]
        
        async def run_sub_agent(name: str, max_retries: int = 2) -> str:
            """执行单个子 Agent（保留原有逻辑）"""
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    logger.warning(f"Sub-agent {name} attempt {attempt + 1} failed: {e}")
                    if attempt == max_retries - 1:
                        logger.error(f"Sub-agent {name} failed after {max_retries} attempts.")
                        return f"Error during {name} analysis: {str(e)}"
                    await asyncio.sleep(1)
            return ""
        
        logger.info("Executing concurrent sub-agents: Security, Performance")
        tasks = [run_sub_agent(name) for name in agent_names]
        ...
        try:
            ...
            # 如果 Reducer 模板加载失败，直接拼接子 Agent 结果
            fallback = self._build_fallback_content(agent_results)
            return self._make_result(AgentStatus.COMPLETED, fallback, start_time)
        ...
        for attempt in range(max_retries):
            try:
                ...
                return self._make_result(AgentStatus.COMPLETED, formatted, start_time)
    ...
    def _format_review_content(self, ai_response: str, owner: str, repo: str, head_sha: str) -> str:
        """
