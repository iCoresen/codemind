...
class CIUpdaterService:
    ...
    async def execute(self, owner: str, repo: str, head_sha: str) -> None:
        ...
        for i, pr in enumerate(prs):
            logger.info(
                f"  PR #{i + 1}: number={pr.get('number')}, title={pr.get('title', 'N/A')[:50]}..."
            )
        ...
        for i, check in enumerate(check_runs):
            ...
            logger.info(
                f"  Check #{i + 1}: name='{name}', status='{status}', conclusion='{conclusion}'"
            )
        ...
        if not relevant_checks:
            ...
            logger.info(
                "所有检查名称: %s", [c.get("name", "unknown") for c in check_runs]
            )
            return
        ...
        for pr_data in prs:
            ...
            # 通过 Aggregator 追加内容
            updated_body = self.aggregator.append_ci_results(
                current_body, relevant_checks, has_failures
            )

            logger.info(
                f"更新前长度: {len(current_body)}, 更新后长度: {len(updated_body)}"
            )

            # 发布更新
            try:
                await self.provider.update_pr_comment(
                    owner, repo, comment_id, updated_body
                )
    ...
    async def _get_prs_for_commit(
        ...
        try:
            resp = await self.provider._client.get(
                url, headers=self.provider._headers()
            )
            ...
            logger.error(
                f"GitHub API错误: {e.response.status_code} - {e.response.text[:200]}"
            )
            return []
        except Exception as e:
            ...
            try:
                ...
                for i, comment in enumerate(comments):
                    ...
                    if "CodeMind" in body or "codemind" in body.lower():
                        ...
                        logger.info(
                            f"  评论 #{i + 1}: 用户={user}, 长度={len(body)}, 匹配为CodeMind评论"
                        )
