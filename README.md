...
4. 若使用 Webhook，需可用 Redis（用于 Celery 与去重锁）
...
处理链路：Webhook 入站 -> Redis 分布式锁去重 -> Celery 异步任务 -> 发布评论 -> 释放锁。
...
4. Worker 没有处理任务
   检查 Redis 可达、`REDIS_URL` 配置正确，并确认 Celery Worker 已启动。
