...
async def test_execute_no_pr_found(ci_updater, mock_github_provider):
    ...
    with patch.object(
        ci_updater, "_get_prs_for_commit", new_callable=AsyncMock
    ) as mock_get_prs:
...
async def test_execute_no_relevant_checks(ci_updater, mock_github_provider):
    ...
    with patch.object(
        ...
        # 模拟没有相关检查
        mock_github_provider.get_pr_check_runs = AsyncMock(
            return_value=[
        ...
        mock_get_prs.assert_called_once_with("owner", "repo", "abc123")
        mock_github_provider.get_pr_check_runs.assert_called_once_with(
            "owner", "repo", "abc123"
        )
...
async def test_execute_no_bot_comments(ci_updater, mock_github_provider):
    ...
    with patch.object(
        ...
        # 模拟没有找到 bot 评论
        with patch.object(
            ci_updater, "_get_bot_comments", new_callable=AsyncMock
    ...
    with patch.object(
        ...
        with patch.object(
            ...
            mock_get_comments.return_value = [
                {
                    "id": 123,
                    "body": "CodeMind review...<!-- CI_RESULTS -->Already appended",
                }
            ]
...
async def test_execute_successful_update(ci_updater, mock_github_provider):
    ...
    with patch.object(
        ...
        with patch.object(
            ...
            mock_aggregator.append_ci_results = MagicMock(
                return_value="Updated body with CI results"
            )
            ci_updater.aggregator = mock_aggregator
...
async def test_execute_multiple_prs(ci_updater, mock_github_provider):
    ...
    with patch.object(
        ...
        with patch.object(
            ...
            mock_github_provider.update_pr_comment.assert_any_call(
                "owner", "repo", 123, "Updated body"
            )
            mock_github_provider.update_pr_comment.assert_any_call(
                "owner", "repo", 456, "Updated body"
            )
...
async def test_get_prs_for_commit_failure(ci_updater, mock_github_provider):
    """测试获取关联 PR 失败"""
    mock_github_provider._client.get = AsyncMock(
...
async def test_get_bot_comments_failure(ci_updater, mock_github_provider):
    ...
    with patch(
        "app.services.ci_updater.asyncio.sleep", new_callable=AsyncMock
    ) as mock_sleep:
        result = await ci_updater._get_bot_comments("owner", "repo", 1)
